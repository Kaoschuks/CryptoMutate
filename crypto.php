...
class Crypto_Mutate {
  ...
  return base64_encode($enc);
    ...
    private function blowfisencrypt($string, $key, $iv) {
        $enc = mcrypt_encrypt(MCRYPT_BLOWFISH, $key, $string, MCRYPT_MODE_CBC, $iv);
        return base64_encode($enc);
    }

    private function blowfisdecrypt($string, $key, $iv) {
        $enc = mcrypt_decrypt(MCRYPT_BLOWFISH, $key, base64_decode($string), MCRYPT_MODE_CBC, $iv);
        return ($enc);
    }
    ...
    private function cbcdecrypt($string, $salt, $key){
        $keys = keys($key, $salt);
        @ $string = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $key, base64_decode($string), MCRYPT_MODE_CBC));
        return $string;
    }
